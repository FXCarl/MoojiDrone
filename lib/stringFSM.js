    ...
    fsm.prototype.generateCallback = function(e){
        this[e.name] = function(){
            ...
            if(!cheakEvent(e.from)){
                console.log('Error fromState!    in  ' + this.name + ' : |'+ e.name + '|  from  |'+ this.current + '|  to  |' + e.to + '|');
                return;
            }
            ...
            //console.log(this.name + " trans  |" + e.name + '|  from  |' + this.current + '|  to  |' +  e.to + '|');
